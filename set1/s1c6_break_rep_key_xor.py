...
def get_first_byte_blocks(in_bytes, keysize):
   block1 = in_bytes[:keysize]
   block2 = in_bytes[keysize:(2 * keysize)]
   return (block1, block2)


def get_byte_blocks(in_bytes, keysize):
    blocks = []
    for start, stop in zip(range(0, len(in_bytes), keysize), range(keysize, len(in_bytes), keysize)):
...
if __name__ == '__main__':
    ...
    in_bytes = base64.b64decode(in_hex)
